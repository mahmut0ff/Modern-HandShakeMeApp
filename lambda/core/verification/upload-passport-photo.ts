// Upload passport verification photo (user holding passport)
...
async function uploadPassportPhotoHandler(
  event: AuthenticatedEvent
): Promise<APIGatewayProxyResult> {
  const userId = event.auth.userId;
  
  logger.info('Upload passport photo request', { userId });
  ...
  try {
    ...
    // Remove old passport photo if exists
    const updatedDocuments = verification.documents.filter(doc => doc.type !== 'PASSPORT_PHOTO');
    
    // Add new passport photo
    ...
    logger.info('Passport photo uploaded successfully', { userId, fileUrl });
    
    const passportPhoto = updatedVerification.documents.find(doc => doc.type === 'PASSPORT_PHOTO');
    ...
    return success({
      ...
      photo: {
        ...
        type: 'passport_photo',
        url: fileUrl,
        uploaded_at: passportPhoto?.uploadedAt,
      },
      verification_status: updatedVerification.status.toLowerCase(),
    });
  } catch (error: any) {
    logger.error('Passport photo upload failed', error);
...
export const handler = withErrorHandler(
  withRequestTransform(
    withAuth(uploadPassportPhotoHandler)
  )
);
