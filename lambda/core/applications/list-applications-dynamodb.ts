...
async function listApplicationsHandler(event: AuthenticatedEvent): Promise<APIGatewayProxyResult> {
  ...
  logger.info('List applications request', { userId, orderId });
  ...
  // Only order owner can list applications
...
export const handler = withErrorHandler(withAuth(listApplicationsHandler, { roles: ['CLIENT'] }));
