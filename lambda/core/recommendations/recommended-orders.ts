...
import { z } from 'zod';
import { success, error, forbidden, notFound } from '../shared/utils/response';
...
const recommendedOrdersHandler = async (event: AuthenticatedEvent): Promise<APIGatewayProxyResult> => {
  ...
  // Check cache first
  const cacheKey = `recommended-orders:${userId}:${JSON.stringify(validatedQuery)}`;
  ...
  // Get master profile
  const masterProfile = await masterProfileRepo.findByUserId(userId);
  ...
  const masterApplications = await applicationRepo.findByMaster(userId);
...
export const handler = withErrorHandler(withAuth(recommendedOrdersHandler, { roles: ['MASTER'] }));
