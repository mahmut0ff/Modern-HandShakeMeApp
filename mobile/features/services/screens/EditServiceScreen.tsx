...
export default function EditServiceScreen() {
  ...
  useEffect(() => {
    if (service) {
      const priceFrom = parseFloat(service.price_from);
      const priceTo = service.price_to ? parseFloat(service.price_to) : undefined;
      ...
      setShowPriceRange(!!priceTo && priceTo > priceFrom);
    }
  }, [service]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateService({ id: serviceId, data: dataToSubmit }).unwrap();
      ...
      Alert.alert('Ошибка', error?.data?.message || 'Не удалось обновить услугу');
    }
  };
  ...
  if (!service) {
    return (
      <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
        <ErrorMessage message="Услуга не найдена" />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-3">
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-lg font-bold text-gray-900">Редактировать</Text>
          <View className="w-10" />
        </View>
      </View>
      ...
      <ScrollView className="flex-1 p-4">
        ...
        <View className="bg-gray-100 rounded-2xl p-4 mb-4">
          <Text className="text-sm text-gray-600 mb-2">
            Создано: {new Date(service.created_at).toLocaleDateString('ru-RU')}
          </Text>
          {service.updated_at && (
            <Text className="text-sm text-gray-600">
              Обновлено: {new Date(service.updated_at).toLocaleDateString('ru-RU')}
            </Text>
          )}
        </View>
      </ScrollView>
