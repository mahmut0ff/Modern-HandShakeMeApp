...
import { deleteUserAccount } from '../../../services/gdprApi';
                  ...
                  onPress: async (password: string | undefined) => {
                    ...
                    try {
                      ...
                      Alert.alert('Аккаунт удалён', 'Ваш аккаунт был успешно удалён', [
                        { text: 'OK', onPress: () => safeNavigate.replace('/(auth)/login') }
                      ]);
                    } catch (error: any) {
                      console.error('Delete account error:', error);
                      Alert.alert('Ошибка', error.response?.data?.message || 'Не удалось удалить аккаунт');
                    } finally {
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 20, paddingTop: 8 }}>
                    ...
                    <TouchableOpacity 
                      onPress={() => {
                        if (item.key === 'change_password') {
                          router.push('/(client)/settings/security');
                        } else if (item.key === 'two_factor') {
                          router.push('/(client)/settings/security');
                        } else if (item.key === 'language') {
                          router.push('/(client)/settings/language');
        ...
        <View className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 mb-4">
          ...
          <View className="flex flex-col gap-1">
            ...
            <TouchableOpacity 
              onPress={() => router.push('/(client)/settings/security')}
              className="flex-row items-center justify-between py-3"
            >
              <View className="flex-1">
            ...
            <TouchableOpacity 
              onPress={() => router.push('/(client)/settings/about')}
              className="flex-row items-center justify-between py-3"
            >
              <View className="flex-1">
