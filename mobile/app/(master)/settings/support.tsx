...
export default function MasterSupportPage() {
  ...
  const supportCategories = [
    ...
    { id: 'client', title: 'Проблемы с клиентом', icon: 'people' },
    { id: 'verification', title: 'Верификация', icon: 'shield-checkmark' },
    { id: 'other', title: 'Другое', icon: 'help-circle' },
  ]

  const handleWhatsApp = () => {
    const phoneNumber = '+996550308078'
    const url = `whatsapp://send?phone=${phoneNumber}&text=Здравствуйте! У меня есть вопрос по приложению HandShakeMe (мастер).`
      ...
      <ScrollView className="flex-1 px-4 pt-4" contentContainerStyle={{ paddingBottom: 20, paddingTop: 8 }}>
        ...
        <View className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 mb-4">
          ...
          <View className="space-y-4">
            {[
              {
                question: 'Как найти заказы?',
                answer: 'Перейдите в раздел "Вакансии", используйте фильтры для поиска подходящих заказов в вашем городе.'
              },
              {
                question: 'Как получить оплату?',
                answer: 'После завершения работы и подтверждения клиентом деньги автоматически поступят на ваш счет в приложении.'
              },
              {
                question: 'Как повысить рейтинг?',
                answer: 'Выполняйте заказы качественно и в срок, получайте положительные отзывы от клиентов.'
              },
              {
